...
Adapted by from https://github.com/nhutter/lkf_tools/blob/main/lkf_tools/dataset.py 
...
class process_dataset(object):
    ...
    def detect_lkfs(self,indexes=None,force_redetect=False):
        ...
        for it in [int(j) for j in self.indexes if j+1 not in self.ind_detect]:
            ...
            if self.latlon:
                lkf = segs2latlon_model(lkf, self.lon.values, self.lat.values)
                                        #np.array(self.lon.values[max([0,self.index_y[0][0]-1]):self.index_y[0][-1]+2:self.red_fac,
                                        #    max([0,self.index_x[0][0]-1]):self.index_x[0][-1]+2:self.red_fac]),
                                        #np.array(self.lat.values[max([0,self.index_y[0][0]-1]):self.index_y[0][-1]+2:self.red_fac,
                                        #    max([0,self.index_x[0][0]-1]):self.index_x[0][-1]+2:self.red_fac]))
            if self.return_eps:
                lkf =  segs2epsvor(lkf, div.values, shr.values, vor.values)
                                #np.array(div.values[max([0,self.index_y[0][0]-1]):self.index_y[0][-1]+2:self.red_fac,
                                #      max([0,self.index_x[0][0]-1]):self.index_x[0][-1]+2:self.red_fac]),
                                #np.array(shr.values[max([0,self.index_y[0][0]-1]):self.index_y[0][-1]+2:self.red_fac,
                                #       max([0,self.index_x[0][0]-1]):self.index_x[0][-1]+2:self.red_fac]),
                                #np.array(vor.values[max([0,self.index_y[0][0]-1]):self.index_y[0][-1]+2:self.red_fac,
                                #       max([0,self.index_x[0][0]-1]):self.index_x[0][-1]+2:self.red_fac]))

            lkf_T = [j.T for j in lkf]
            #np.save(self.lkfpath.joinpath('lkf_%s_%03i.npy' %(self.netcdf_file.split('/')[-1].split('.')[0],(it+1))), lkf_T)
            with open(self.lkfpath.joinpath('lkf_%s_%03i.pkl' %(self.netcdf_file.split('/')[-1].split('.')[0],(it+1))), 'wb') as f:
                pickle.dump(lkf_T, f)
