""" 
Adapted from https://github.com/nhutter/lkf_tools/blob/main/lkf_tools/stats.py 
...
#            self.indd0 = 4+1; self.indd1 = 5+1;
        ...
        if self.mask_rgps:
            ...
            elif self.datatype == 'mitgcm_4km':
                cov= np.load('/scratch/users/evlema001/coverage_rgps_2008.npz')
            else:
                cov = np.load('/work/ollie/nhutter/sirex/analysis/lead_detect/RGPS/coverage_rgps_1997_mod.npz')
        ...
        for year_dic in years:
            ...
            if self.mask_rgps:
                if self.datatype=='mitgcm_2km':
                    ...
                    cov_year = np.load('/scratch/users/evlema001/coverage_rgps_2008.npz')['coverage']
                else:
            ...
            for it,lkffile in enumerate(lkffile_list):
                ...
                elif lkffile.endswith('.pkl'):
                    
                    with open(new_dir+lkffile,'rb') as f :
                         lkfi = pickle.load(f)
                    lkf_meta_year.append(np.array([startdate,
                                                   enddate,
                                                   #lkfi.size]))
                    ...
                    if self.mask_rgps:
                        ...
                        if polyfit:
                            isegf = np.rollaxis(np.stack(lkf_poly_fit(lkfim[-1][:,self.indm0],
                                                                      ...
                                                                      poly_deg)),1,0)
                            lkfim[-1] = np.concatenate([lkfim[-1], isegf], axis=1)
                #if lkfim[0].shape[1]!=11:
                #    lkfim_T = [j.T for j in lkfim]
                #else :
                #    lkfim_T = lkfim
                with open(lkf_path + 'Masked_' + lkffile_list[it] , 'wb') as f:
                    pickle.dump(lkfim, f)
            ...
            if read_tracking:
                ...
                track_dir = os.path.join(lkf_path,track_dir_name)
                # Generate list of files
...
class lkf_lengths:    
    ...
    def fit_stretched_exponential(self,bins=np.linspace(50,1000,80),xmin=100,xmax=1000,mctest=True,mctest_plots=False,mciter=1000):
        ...
        if mctest:
            ...
            print('ks of data is smaller than 95 percentile of Monte Carlo: %r' %(ks_org<np.percentile(ks_list,95)))
...
class lkf_deformation:
    def __init__(self,lkf):
        ...
        #Save output
        self.lkf_deformation = lkf_deformation
...
class lkf_intersection:
    def __init__(self,lkf,pos_type='poly',num_p = 10,link_def_life_len=True,link_def_len=False,use_vorticity=True,lkf_path=None,years=None,dis_par=1):
        ...
        if self.use_vorticity:
            self.lkf_interc_type = lkf_interc_type
    ...
    def plot_hist_def_len(self,def_class=None,bins=np.linspace(0,90,23),
        ...
        if def_class is None:
            if datatype=='rgps':
                def_class = [0,0.03,0.1,2]
            elif datatype == 'mitgcm_2km' or datatype == 'mitgcm_4km':
                def_class = [0,0.05,0.2,10]
        if gen_fig:
            style_label = 'seaborn-v0_8-darkgrid'
            with plt.style.context(style_label):
...
class lkf_orientation:
    def __init__(self,lkf,res=200e3,use_poly_ori=True):
        ...
        self.cell_contrib = cell_contrib
        ...
        #Save output
        self.lkf_angle = lkf_angle
        self.lkf_angle_len_wght = lkf_angle_len_wght
        self.lkf_orientation = lkf_orientation
        self.lkf_ori_len_wght = lkf_ori_len_wght
